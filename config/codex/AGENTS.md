...
- ALWAYS because a different model has a different training signal and catches
  things you miss, get a second opinion from Claude Code (or another model) at
  the moments I would otherwise want a tech lead or engineering manager to
  review the work: architectural decisions, risky diffs, migrations,
  security-sensitive code, or when stuck for 20+ minutes. Routine edits do not
  need cross-review.
